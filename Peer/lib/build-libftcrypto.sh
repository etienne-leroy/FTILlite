...
docker run --rm -v"$PWD:/src" "nvidia/cuda:${CUDA_VERSION}-devel-ubuntu20.04" nvcc --compiler-options '-fPIC' --shared -arch "${CUDA_ARCH}" -o /src/out/libftcrypto.so /src/src/ftcrypto.cu
