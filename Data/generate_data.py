...
def generate_account_numbers(G):
    # Re-label nodes with 15 digit account numbers (bsb = 6 digits +
    # acc_num = 9 digits)
    acc_num = config['Graph'].getint('account_num_len', fallback=15)
    acc_num_sample = np.random.randint(
        10**(acc_num-1), high=(10**(acc_num) - 1), size=G.number_of_nodes())
    ...
    gen_count = bsb_num - len(str(len(peer_names)))

    for i, p in enumerate(peer_names):
        prefix = (i+1) * 10**(gen_count)
        bsb_map[p] = prefix + np.random.randint(
        10**(gen_count-1), high=(10**(gen_count) - 1), size=num_bsbs_per_peer[p])
    
    # Randomly assign a peer name for each graph node (the fraction assigned to
    # each peer can be modified easily here if necessary)
    for n in G:
        # use str() to convert from np.str_ to str
        G.nodes[n]['Peer'] = str(np.random.choice(peer_names))
        G.nodes[n]['BSB'] = str(np.random.choice(bsb_map[G.nodes[n]['Peer']]))
...
def generate_transactions(G, output_filename, avg_num_tx_per_pair, avg_tx_amount, sigma_tx_amount, start_date_str, date_range, start_time, end_time):
    ...
    with open(output_filename, 'w') as tx_file:
        tx_file.write("origin_re,origin_id,origin_bsb,dest_re,dest_id,dest_bsb,total_amount,transaction_date\n")
        ...
        for source, dest in G.edges():
            ...
            sourceBSB = G.nodes[source]['BSB']
            destRE = G.nodes[dest]['Peer']
            destBSB = G.nodes[dest]['BSB']
            # write_transactions() will write n_tx transactions from
            # source to dest to the file-hanle tx_file
            write_transactions(
                source, dest, sourceBSB, destBSB, sourceRE, destRE, tx_amounts, tx_dates, tx_file)
        # Text files should end with a new line character
        tx_file.write('\n')


def write_transactions(source, dest, sourceBSB, destBSB, sourceRE, destRE, tx_amounts, tx_dates, tx_file):
...
def write_accounts(G, dir_name, acc_type, acc_type_probs, cust_occupation, cust_occupation_probs):
    ...
    header = "re,account_id,bsb,account_open_date,account_type,cust_occupation\n"
...
if __name__ == '__main__':
    ...
    elif len(num_bsbs) == len(peer_names):
    ...
    print(f'num_bsbs_per_peer: {num_bsbs_per_peer}')
    ...
    for p in peer_names:
        ...
        os.system(f"echo 'origin_re,origin_id,origin_bsb,dest_re,dest_id,dest_bsb,total_amount,transaction_date' > {peer_output_filename}")
